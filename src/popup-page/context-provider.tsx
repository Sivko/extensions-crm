...
async function init({ setAccount, setAddress, setToken }: { setAccount: SetStateAction<any>, setAddress: SetStateAction<any>, setToken: SetStateAction<any> }) {
...
async function updateAddressAndToken({ address, token, setAccount, setOptions, setTest, setFlagUpdate, flagUpdate }: { address: string, token: string, setAccount?: SetStateAction<any>, setOptions: SetStateAction<any>, setTest: SetStateAction<any>, flagUpdate: boolean, setFlagUpdate: SetStateAction<any> }) {
  try {
    if (flagUpdate) {
      const options = {
      ...
      setAccount({userData, tokenData})
      setTest({userData, tokenData})
      chrome.storage.local.set({"account": {tokenData, userData} })
      console.log({"account": {tokenData, userData} })
      setTest({"account": {tokenData, userData} })
    }
    setFlagUpdate(true);
  } catch (error:any) { 
...
function ContextProvider({ children }: Readonly<{ children: React.ReactNode; }>) {
  ...
  useEffect(() => {
    init({ setAccount, setToken, setAddress })
  }, [])

  useEffect(() => {
    if (token) {
      updateAddressAndToken({ address, token, setOptions, setTest, setFlagUpdate, flagUpdate })
    }
